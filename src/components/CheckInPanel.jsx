...
import { fetchCheckInSettings } from '../services/gistSettingsApi';
    ...
    const syncSettings = async () => {
      try {
        ...
        if (result.success && result.settings) {
          ...
          console.log('✅ 已從 Gist 同步報到設置');
        }
      } catch (error) {
        console.error('❌ 同步報到設置失敗:', error);
      }
    };
    
    syncSettings(); // 立即執行一次
    const interval = setInterval(syncSettings, 10000); // 每 10 秒
